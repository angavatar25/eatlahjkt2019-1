<nav class="navbar navbar-expand-md fixed-top">
    <div class="container-fluid">
      <a class="navbar-brand d-inline-block d-md-none" href="<?php echo $prefix;?>">
        <img src="<?php echo $prefix;?>assets/img/brand/logo_eatlah-white.png" class="img-fluid" alt="Logo">
      </a>
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarCentered" aria-expanded="false" aria-label="Toggle navigation">
        <span class="icon-bar"></span>
        <span class="icon-bar"></span>
        <span class="icon-bar"></span>
      </button>

      <div class="collapse navbar-collapse navbar-collapse-centered" id="navbarCentered">
        <ul class="navbar-nav navbar-nav-left">
          <li class="nav-item active">
            <a class="nav-link smooth-scroll" href="<?php echo $prefix;?>#about">About</a>
          </li>
          <li class="nav-item">
            <a class="nav-link smooth-scroll" href="<?php echo $prefix;?>#menu">Menu</a>
          </li>
        </ul>
        <a class="navbar-brand d-none d-md-inline-block" href="<?php echo $prefix;?>">
          <img src="<?php echo $prefix;?>assets/img/brand/logo_eatlah-white.png" class="logo-center" alt="Logo">
        </a>
        <ul class="navbar-nav navbar-nav-right">
          <li class="nav-item">
            <a class="nav-link smooth-scroll" href="<?php echo $prefix;?>#location">Locations</a>
          </li>
          <li class="nav-item">
            <a class="nav-link smooth-scroll" href="<?php echo $prefix;?>#contact">Contact Us</a>
          </li>
        </ul>
      </div>
    </div>
  </nav>